...
<script>
	function sure(){
		var delete_product = confirm("Sure you want to cancel your order?");
                            ...
                            <div class="table_page table-responsive">
                                <table>
                                    ...
                                    <tbody>
										...
										<?php
											$show = $link->rawQuery("select * from order_product join order_item on order_product.order_product_id = order_item.order_product_id join product on order_item.product_id = product.product_id join product_price on order_item.product_id = product_price.product_id where order_product.user_id=?",array($user_id));
											if($link->count > 0){
												foreach($show as $s){
											
										?>
                                        <tr>
											<form name="orders" method="post" action="cancel-order.php?order_product_id=<?php echo $s['order_product_id'];?>" onSubmit="sure();">
                                            ...
                                            <td class="product_quantity pQuantity" onChange="subtotal()"><label></label><?php echo $s['quantity'];?></td>
										...
										<td class="cart_submit">
											<button type="submit" class="btn btn-md btn-golden" name="update" id="update">Cancel Order</button>
										</td>
										</tr>
										</form>
