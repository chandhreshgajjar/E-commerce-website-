...
<!-- Mirrored from htmldemo.net/hono/hono/cart.html by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 12 Oct 2022 11:23:49 GMT -->
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />

    <title>Cart|Nudra</title>
...
<script>
	function checkCoupon(){
		...
		var b = document.cf.coupon.value;
		
		if(b==0){
			document.getElementById("c1").innerHTML = "Enter Coupon Name";
...
<body>
				...
				<div class="col-12">
                    <div class="user-actions accordion" data-aos="fade-up" data-aos-delay="0">
                        <h3>
                            ...
                            Please Login in to see your cart products.
                            <a class="Returning" href="login.php">Click here to login</a>
                        </h3>
                            ...
                            <div class="table_page table-responsive">
                                <table>
                                    ...
                                    <tbody>
										...
										<?php
											$show = $link->rawQuery("SELECT * FROM cart JOIN product ON cart.product_id = product.product_id join product_price ON cart.product_id = product_price.product_id JOIN user ON cart.user_id = user.user_id where user.user_id=?",array($user_id));
											if($link->count > 0){
												foreach($show as $s){
											
										?>
                                        <tr>
											<form name="cart" method="post" action="update-cart.php?cart_id=<?php echo $s['cart_id'];?>">
                                            <td class="product_remove"><a href="delete-product.php?cart_id=<?php echo $s['cart_id'];?>"><i class="fa fa-trash-o"></i></a>
                                            </td>
                                            <td class="product_thumb"><a href="#">
											<img src="<?php echo $s['product_image']?>" alt=""></a></td>
                                            <td class="product_name" name="product_name"><a href="#"><?php echo $s['product_name'];?></a></td>
                                            <td class="total-price pPrice"> <input type="hidden" value="<?php echo $s['product_price']?>">Rs.<?php echo $s['product_price']?></td>
                                            <td class="product_quantity pQuantity" onChange="subtotal()"><label></label> <input min="1"
                                                    max="100" value="<?php echo $s['quantity']?>" type="number" name="quantity"></td>
                                            <td class="product_total pTotal"><?php echo $total=$s['product_price']*$s['quantity'];?></td>
											<td class="cart_submit">
												<button type="submit" class="btn btn-md btn-golden" name="update" id="update">update cart</button>
											</td>
											</form>
                                        </tr> 
										<?php		
                    ...
                    <div class="col-lg-6 col-md-6">
                        <div class="coupon_code left" data-aos="fade-up" data-aos-delay="200">
                            ...
                            <div class="coupon_inner">
								<form name="cf" method="POST" onSubmit="checkCoupon();">
                                <p>Enter your coupon code if you have one.</p>
                                <input class="mb-2" placeholder="Coupon code" type="text" id="coupon" name="coupon">
								<span id="c1" style="color:red"></span>
								
                                <button type="submit" id="submit" name="submit" class="btn btn-md btn-golden">Apply coupon</button>
                            ...
                            <div class="coupon_inner">
								...
								<div class="cart_subtotal">
									...
									<?php
										...
										$coupon_name = $_REQUEST['coupon'];
																				
										$check = $link->rawQueryOne("select * from coupon where coupon_name='$coupon_name'");
										
											if($check){
												
												$coupon_id = $link->rawQueryOne("select coupon_id from coupon where coupon_name='$coupon_name'");
												
												if($link->count > 0){
													foreach($coupon_id as $cid){
												
												$update = $link->rawQueryOne("update cart set coupon_id='$cid', coupon_name='$coupon_name' where user_id=?",array($user_id));
												
													}
												}
												$amount = ($t*$check['discount'])/100;
												$t=$t-$amount;
									?>
									...
									?>
									<span style="color:red">Incorrect Coupon Code.</span>
									<?php
    ...
    let price = document.getElementsByClassName('pPrice');
    let quantity = document.getElementsByClassName('pQuantity');
    let total = document.getElementsByClassName('pTotal');
